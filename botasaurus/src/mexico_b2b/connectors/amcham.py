"""
AmCham Mexico (American Chamber of Commerce of Mexico) Connector.
Ingests enterprise profiles, B2B services, and leadership contacts from AmCham Mexico Trade & Investment Center.
"""

import json
from pathlib import Path
from typing import List, Dict, Any, Optional
from .base import SourceConnector
from ..config.settings import settings
from ..models.company import CanonicalCompany, Address, PhoneItem, EmailItem
from ..models.source_record import RawSourcePayload, SourceProvenanceRecord
from ..pipeline.normalization import (
    normalize_company_name,
    normalize_email,
    normalize_website,
)
from ..utils.address_utils import normalize_state, clean_postal_code
from ..utils.phone_utils import format_mx_phone_e164
from ..utils.rfc_utils import clean_rfc, is_valid_rfc, get_rfc_type
from ..utils.hashing import sha256_dict, generate_entity_fingerprint, generate_company_id
from ..storage.raw_storage import raw_storage
from ..utils.logging import logger


class AmchamConnector(SourceConnector):
    """
    Connector for AmCham Mexico Trade & Investment Center and member directory.
    Captures binational corporate members, industrial manufacturers, and B2B services.
    """

    def fetch(self, limit: Optional[int] = None) -> List[RawSourcePayload]:
        raw_payloads: List[RawSourcePayload] = []
        target_limit = limit or 100

        fixture_path = settings.PROJECT_ROOT / "tests" / "fixtures" / "sample_amcham.json"
        remote_url = self.config.raw_config.get("portal_url") or self.config.url or "https://amcham.org.mx/trade-and-investment-center/"

        if fixture_path.exists():
            logger.info(f"Loading AmCham Mexico records from fixture: {fixture_path.name}")
            with open(fixture_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            for item in data[:target_limit]:
                row_id = str(item.get("id") or item.get("rfc") or len(raw_payloads) + 1)
                raw_payloads.append(
                    RawSourcePayload(
                        source="AMCHAM",
                        source_record_id=row_id,
                        source_url=item.get("url") or str(remote_url),
                        raw_data=item,
                        raw_hash=sha256_dict(item),
                    )
                )
            return raw_payloads

        if remote_url and str(remote_url).startswith("http"):
            try:
                logger.info(f"Fetching AmCham directory from {remote_url}")
                response = self.http_client.get(remote_url)
                raw_storage.save_raw_text(
                    source_name="AMCHAM",
                    filename="amcham_directory.html",
                    text_content=response.text,
                    source_url=remote_url,
                )
            except Exception as e:
                logger.warn(f"AmCham live fetch notice: {str(e)}")

        return raw_payloads

    def parse(self, payload: RawSourcePayload) -> Dict[str, Any]:
        d = payload.raw_data
        return {
            "amcham_id": payload.source_record_id,
            "rfc": d.get("rfc") or d.get("RFC"),
            "legal_name": d.get("company_name") or d.get("razon_social"),
            "trade_name": d.get("trade_name") or d.get("nombre_comercial"),
            "industry": d.get("business_sector") or d.get("sector") or d.get("description"),
            "street": d.get("address_street") or d.get("street") or "",
            "number": d.get("address_number") or d.get("number") or "",
            "colony": d.get("neighborhood") or d.get("colonia") or "",
            "municipality": d.get("city") or d.get("municipio") or "",
            "state": d.get("state") or d.get("estado") or "",
            "postal_code": d.get("postal_code") or d.get("cp") or "",
            "phone": d.get("company_phone") or d.get("phone") or d.get("executive_phone"),
            "email": d.get("company_email") or d.get("email") or d.get("executive_email"),
            "website": d.get("website") or d.get("url"),
            "executive_name": d.get("executive_name") or d.get("contact_name"),
            "executive_title": d.get("executive_title") or d.get("contact_title") or "Managing Director",
            "executive_email": d.get("executive_email"),
            "executive_phone": d.get("executive_phone"),
            "member_tier": d.get("tier") or "Corporate Member",
        }

    def normalize(self, parsed: Dict[str, Any], provenance: SourceProvenanceRecord) -> CanonicalCompany:
        legal_orig, legal_norm = normalize_company_name(parsed.get("legal_name"))
        trade_orig, trade_norm = normalize_company_name(parsed.get("trade_name"))

        primary_norm_name = legal_norm or trade_norm or ""
        clean_rfc_val = clean_rfc(parsed.get("rfc"))
        rfc_valid = is_valid_rfc(clean_rfc_val)
        rfc_type_val = get_rfc_type(clean_rfc_val) if rfc_valid else None

        norm_state = normalize_state(parsed.get("state"))
        clean_cp = clean_postal_code(parsed.get("postal_code"))
        norm_url, domain = normalize_website(parsed.get("website"))
        clean_em = normalize_email(parsed.get("email"))
        phone_e164 = format_mx_phone_e164(parsed.get("phone"))

        address = Address(
            street=parsed.get("street"),
            number=parsed.get("number"),
            colony=parsed.get("colony"),
            municipality=parsed.get("municipality"),
            state=norm_state or parsed.get("state"),
            postal_code=clean_cp,
            country="Mexico",
        )

        phones = [PhoneItem(value=phone_e164, source="AMCHAM")] if phone_e164 else []
        emails = [EmailItem(value=clean_em, source="AMCHAM")] if clean_em else []

        fp = generate_entity_fingerprint(
            rfc=clean_rfc_val if rfc_valid else None,
            normalized_name=primary_norm_name,
            state=address.state,
            municipality=address.municipality,
            domain=domain,
        )
        comp_id = generate_company_id(fp)

        company = CanonicalCompany(
            company_id=comp_id,
            legal_name=legal_orig,
            trade_name=trade_orig,
            normalized_name=primary_norm_name,
            rfc=clean_rfc_val if rfc_valid else None,
            rfc_type=rfc_type_val,
            website=norm_url,
            domain=domain,
            industry=parsed.get("industry"),
            phone=phone_e164,
            phones=phones,
            email=clean_em,
            emails=emails,
            address=address,
            source_records=[provenance],
            source_count=1,
            entity_fingerprint=fp,
            last_verified_at=provenance.retrieved_at,
        )
        return company
